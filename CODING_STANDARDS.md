...
- **`marketdata` defines its own `HistoricalCandleFeed` interface** (pull-based: fetch a candle range) rather than implementing `session.MarketDataFeed` (push-based: live ticks) — the two solve different problems and forcing one onto the other would be a worse fit than two small interfaces.
- **`marketdata` -> `instrument` is a one-way edge, added for story #24.** `marketdata.pricepath.HistoricalPricePathSource` takes an `Optional<InstrumentCatalog>` and refuses instrument keys outside today's tracked NIFTY-options window, the same split `live.TradeMonitor.isOutOfScope` uses for live fills: the broker client (`HistoricalCandleFeed`/`UpstoxHistoricalCandleFeed`) stays generic, the orchestration seam one level up enforces product scope. `instrument` has no reverse dependency on `marketdata`, so this doesn't create a cycle. Nothing in `live`, `engine`, or `session` depends on `marketdata` yet — backtesting (`simulation.runner`) consumes its output as a plain `List<Double>`, the same shape `StochasticPricePathGenerator` already produces, so no new edge was needed there either.
